...
export class StarkScanClient extends AbstractDataClient implements IReadDataClient {
  ...
  async getRawTxns(address: string,
    pageSize: number,
    ...
    try {
        ...
        let url = nextUrl === null ? `${this.baseUrl}/transactions?contract_address=${address}&limit=${pageSize}&order_by=${orderBy}`: nextUrl;

        logger.info(`[StarkScanClient] getRawTxns url: ${url}`)
        const response = await fetch(url, {
            method: "GET",
            headers : this.getCredentialHeader(),
        });
        logger.info(`[StarkScanClient] getRawTxns status: ${response.ok}`)
  ...
  async getTxns(
    ...
    try {
        ...
        while (process) {
            ...
            logger.info(`[StarkScanClient] getTxns data, ${JSON.stringify(data, null, 2)}`)
            for (const item of data) {
                if (item.timestamp * 1000 >= minTimestamp) {
