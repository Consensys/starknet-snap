...
export class StarknetTransactionManager implements ITransactionMgr {
  ...
  async list(address: string, chainId: string, tokenAddress: string, minTimestamp?: number): Promise<Transaction[]> {
    try {
      ...
      logger.info(
        `[StarknetTransactionManager.list] txns: ${txns.length} found,  deployTxn: ${deployAccountTxn ? 1 : 0} found`,
      );

      logger.info(
        `[StarknetTransactionManager.list] filter txns on contractAddress = ${tokenAddress}, chainId = ${chainId}, timestamp >= ${minTimestamp}`,
      );
  ...
  protected async fetchStatus(txns: Transaction[]): Promise<void> {
    try {
      ...
      logger.info(
        `[StarknetTransactionManager.fetchStatus] ${txnsNeededStatus.length} transactions need status update`,
      );
