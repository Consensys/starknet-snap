import { StarknetWindowObject, type RpcMessage, type WalletEvents } from 'get-starknet-core';
...
export class MetaMaskSnapWallet implements StarknetWindowObject {
  ...
  readonly snap: MetaMaskSnap;
  readonly state: MetaMaskSnapWalletState;

  private static readonly SNAPI_ID = 'npm:@consensys/starknet-snap';

  constructor(protected readonly metamaskProvider: MetaMaskProvider, snapVersion = '*') {
    this.id = 'metamask';
    this.name = 'Metamask';
    this.version = 'v2.0.0';
    ...
    this.state = new MetaMaskSnapWalletState();
  }
  ...
  async request<T extends RpcMessage>(call: Omit<T, 'result'>): Promise<T['result']> {
    const rpcHandlers: Record<string, StaticRPCHandler> = {
      ...
      wallet_requestAccounts: WalletRequestAccounts,
